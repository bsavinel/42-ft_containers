...
namespace ft
{
	...
	class RBT_set
	{
		...
		private:
			typedef typename ft::Iterator_set<value_type, Alloc>	iterator;
		...
		public:
			...
			node	*find_key(const key_type key) const
			{
				...
				while (tmp != _sentinel)
				{
					if (_comp(key, tmp->_value)) //? true a < b
						tmp = tmp->_left;
			...
			ft::pair<node *,bool>	insert_value(const value_type &data)
			{
				...
				while (tmp != _sentinel)
				{
					y = tmp;
					this->setSentinel();
					if (_comp(data, tmp->_value)) //? true a < b
						tmp = tmp->_left;
					else if (_comp(tmp->_value, data))
						tmp = tmp->_right;
					else
				...
				else if (_comp(newNode->_value, y->_value))
					y->_left = newNode;
				else
					y->_right = newNode;
				newNode->_parent = y;
